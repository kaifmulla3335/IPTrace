...
function DetailRow({ label, value, mono }) {
  return (
    <div className="flex items-center justify-between py-2.5 border-b border-white/4 last:border-0">
      <span className="text-[11px] text-slate-500 uppercase tracking-widest font-display">{label}</span>
    ...
    >
      ...
      {isVPN && (
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="inline-flex items-center gap-2 bg-amber-400/10 border border-amber-400/25 rounded-full px-4 py-1.5 mb-5"
        >
          <ShieldIcon />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
        {/* IP */}
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="relative">
          <InfoCard
            title="IP Address"
            value={ipData.ip}
            accent="cyan"
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            onClick={onCopy}
            className={`absolute top-4 right-4 flex items-center gap-1.5 text-[11px] px-2.5 py-1.5 rounded-lg font-semibold transition-all duration-200 z-10 ${
              copied
                ? 'bg-emerald-500/20 text-emerald-400 border border-emerald-500/30'
                : 'bg-white/6 text-slate-400 hover:text-white border border-white/8 hover:border-white/15'
            }`}
          >
        ...
        <motion.div initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
          <InfoCard
            title="Location"
            value={formatLocation(ipData.location)}
            accent="violet"
            badge={ipData.location?.country_code}
          />
        </motion.div>
            ...
            >
              <div className="mt-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="glass rounded-2xl p-5 border border-white/6 text-left">
                  <p className="text-[10px] text-slate-500 uppercase tracking-[0.18em] font-display mb-3">Network</p>
                ...
                <div className="glass rounded-2xl p-5 border border-white/6 text-left">
                  <p className="text-[10px] text-slate-500 uppercase tracking-[0.18em] font-display mb-3">Security Flags</p>
